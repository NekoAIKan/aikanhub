...
Known prior offenders fixed: `relay/image_audit_error.go`, `controller/image_audit.go`, `service/error.go::ClaudeErrorWrapper` (now masks URLs instead of replacing with `"请求上游地址失败"`), `service/error.go::RelayErrorHandler` (always includes truncated+masked body when JSON parse fails, regardless of `showBodyWhenFail`), `relay/relay_task.go:242` (`DoRequest` failure now returns 502, not 500 — that's the literal HTTP semantics of "gateway can't reach upstream"). Known unfixed: `relay/relay_task.go:236` (BuildRequestBody hardcoded 500; image-audit branch already passes through but generic adaptor build errors still bucket as 500 — needs per-adaptor analysis), task-adaptor "upstream 2xx but body unparseable" 500/502 sites in `relay/channel/task/{pixverse,sora,doubao,gemini}/adaptor.go` (defensible as 5xx since we genuinely failed to parse, but should include upstream body in message). Fix incrementally; the rule is the gate for new code.
