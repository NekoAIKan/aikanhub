...
Known prior offenders fixed: `relay/image_audit_error.go`, `controller/image_audit.go`, `service/error.go::ClaudeErrorWrapper` (now masks URLs instead of replacing with `"请求上游地址失败"`), `service/error.go::RelayErrorHandler` (always includes truncated+masked body when JSON parse fails, populates BOTH `Err` and `RelayError.Message` — see the live-verification trap below), `relay/relay_task.go:242` (`DoRequest` failure now returns 502, not 500), `relay/channel/api_request.go:526` (`DoApiRequest` transport failure no longer uses `types.ErrOptionWithHideErrMsg("upstream error: do request failed")` — that quietly destroyed every Claude/OpenAI/Gemini/etc. dial-error before reaching the client, including DNS/connect-refused/timeout root-causes. Now uses `MaskSensitiveInfo` + 502 like the task path. Replaces the original wrapper anti-pattern with the same passthrough policy).

**Live-verification trap caught during PR #73:** the response serializer (`types.NewAPIError.ToOpenAIError`) reads `e.RelayError.(OpenAIError).Message`, NOT `e.Err`. A fix that only sets `.Err` passes a naive `apiErr.Err.Error()` assertion but the wire response still falls back to the empty-message default "openai_error". Always assert against `apiErr.ToOpenAIError().Message` in tests, and use the `types.NewOpenAIError`/`types.WithOpenAIError` constructors which populate both fields. Discovered when curl returned `{"message":"openai_error"}` against a mock 503 — unit tests had asserted only `apiErr.Err`.

Known unfixed (tracked as separate issues, fix incrementally one adaptor per PR): #74 `relay/relay_task.go:236` BuildRequestBody hardcoded 500 — image-audit branch already passes through, but generic adaptor build errors still bucket as 500 and need per-adaptor user-input vs internal classification. #75 task-adaptor "upstream 2xx but body unparseable" 500/502 sites in `relay/channel/task/{pixverse,sora,doubao,gemini}/adaptor.go` — defensible as 5xx (we genuinely failed to parse), but message must include the upstream body so operators and customers can act. The rule is the gate for new code.
