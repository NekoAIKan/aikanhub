...
func setupLogin(user *model.User, c *gin.Context) {
	...
	c.JSON(http.StatusOK, gin.H{
		...
		"data": map[string]any{
			...
			"status":                  user.Status,
			"group":                   user.Group,
			"billing_visibility_mode": service.GetBillingVisibilityMode(user.Role, user.Group),
		},
	})
}
...
func applyRegistrationInvitePolicy(tx *gorm.DB, user *model.User, inviteCode string, allowAffiliateFallback bool, requireCampaign bool) (int, error) {
	...
	if err == nil {
		if campaign.Quota >= 0 {
			user.OnboardingQuota = &campaign.Quota
		}
		if campaign.Group != "" {
	...
	inviterId, _ := model.GetUserIdByAffCode(inviteCode)
...
func GetAllUsers(c *gin.Context) {
	...
	annotateUserBillingVisibility(users)
...
func GetUser(c *gin.Context) {
	...
	annotateUserBillingVisibility([]*model.User{user})
	c.JSON(http.StatusOK, gin.H{
		"success": true,
		"message": "",
		"data":    user,
...
func GetSelf(c *gin.Context) {
	...
	responseData := map[string]interface{}{
		...
		"group":                   user.Group,
		"billing_visibility_mode": service.GetBillingVisibilityMode(userRole, user.Group),
		"quota":                   user.Quota,
...
func annotateUserBillingVisibility(users []*model.User) {
	for _, user := range users {
		if user == nil {
			continue
		}
		user.BillingVisibilityMode = service.GetBillingVisibilityMode(user.Role, user.Group)
	}
}
